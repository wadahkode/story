...
class RealtimeComment {
  ...
  async saveComment() {
    if (
      localStorage.getItem("dataComment") &&
      localStorage.getItem("keyboardHistory")
    ) {
      this.data = JSON.parse(localStorage.getItem("dataComment"));
      this.keyboardHistory = JSON.parse(
        localStorage.getItem("keyboardHistory")
      );
    }
    ...
    localStorage.setItem(
      "keyboardHistory",
      JSON.stringify(this.keyboardHistory)
    );
    ...
    const context = setInterval(() => {
      ...
      if (this.keyboardHistory.length >= 1) {
        this.keyboardHistory.map((keyboard) => {
          ...
          this.totalComments[keyboard.id].innerHTML =
